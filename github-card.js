  ...
  document.querySelectorAll('.repo-card').forEach(async function (el) {
    ...
    el.innerHTML = `
      ...
      <div class="repo-card>
        ...
        <div style="display: ${data.fork ? 'block' : 'none'}; font-size: 12px;">Forked from <a style="color: inherit; text-decoration: none;" href="${data.fork ? data.source.html_url : ''}" target="_blank" >${data.fork ? data.source.full_name : ''}</a></div>
        <div style="font-size: 12px; margin-bottom: 16px; margin-top: 8px;">${data.description}</div>
